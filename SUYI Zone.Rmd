...
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, collapse=FALSE, highlight= FALSE, results='asis', fig.height = 4, fig.width = 7,  fig.align = "center")
```
...
df_long %>% 
  ggplot(aes(subject, rit_score, fill= semester)) + scale_fill_manual(values=c("black","#AA0000", "grey")) + labs(title= "Average RIT Score by Semester" ,x = "Subject", y = "Average Score", fill= "Group") + geom_col(position = "dodge")  + scale_y_continuous(breaks = c(0,50,100,150,200,250,300)) #+ coord_flip() + facet_wrap(~semester)
```
...
df_long %>% 
  ggplot(aes(subject, rit_score, fill= suyi)) + scale_fill_manual(values=c("black","#AA0000")) + labs(title= "Average RIT Score by Sex" ,x = NULL, y = "Average Score", fill= "Group") + geom_col(position = "dodge") + facet_wrap(~sex) + scale_y_continuous(breaks = c(0,50,100,150,200,250,300)) + coord_flip()
```
...
-Make a graphs showing the race of all the students 
